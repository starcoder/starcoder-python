...
if __name__ == "__main__":
    ...
    splits = [(f, int((p / total) * len(components))) for f, p in zip(args.outputs, args.proportions)]

    for fname, c in splits:
        logging.info("Writing %d components to %s", c, fname)
        with gzip.open(fname, "wb") as ofd:
            pickle.dump(components[:c], ofd)
