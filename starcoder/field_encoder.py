...
class SequenceEncoder(FieldEncoder):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        #print(l.sum() / float(x.flatten().shape[0]))
        buckets = torch.bucketize(l, torch.tensor([i * bucket_size for i in range(bucket_count)]))
        space = 0
        actual = 0
        for bucket in buckets.unique():
            ...
            #subset_mask = subset_nonempty.unsqueeze(1).expand((subset_nonempty.shape[0], subset_h.shape[1]))
            subset_mask = bucket_mask.unsqueeze(1).expand((bucket_mask.shape[0], subset_h.shape[1]))
            #print(subset_mask.shape)
            retval.masked_scatter_(subset_mask, subset_h)

            #space += subset.flatten().shape[0]
            #actual += subset_l.sum()
            #print(subset, subset.shape)
            #print(subset, bucket, bucket_size)
        #print(actual / float(space))
        #sys.exit()
        return retval
